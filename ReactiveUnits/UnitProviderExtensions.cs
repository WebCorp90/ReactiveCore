...
namespace Webcorp.unite
{
    public static class UnitProviderExtensions
    {
        ...
        public static void RegisterUnits(this IUnitProvider unitProvider, System.Reflection.Assembly assembly)
        {
            ...
            {
#if CORE
                if (typeof(IUnit).GetTypeInfo().IsAssignableFrom(t))
#else
        ...
        {
#if CORE
            foreach (var property in type.GetRuntimeProperties().Where(p=>p.GetMethod.IsStatic && p.GetMethod.IsPublic))
#else
...
#if CORE
            foreach (var property in type.GetRuntimeFields().Where(f => f.IsStatic && f.IsPublic))
#else
